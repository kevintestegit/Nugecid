...
async function executeSqlStatements() {
  ...
  try {
    ...
    console.log(
      "   ✅ Conectado:",
      r[0].version.split(" ").slice(0, 2).join(" "),
    );
  } catch (e) {
  ...
  for (let i = 0; i < fullSql.length; i++) {
    ...
    if (char === "$" && !inString && !inDollarQuote) {
      ...
      inDollarQuote &&
      fullSql.substring(i, i + dollarTag.length) === dollarTag
    ) {
      inDollarQuote = false;
      current += dollarTag;
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    if (stmt.includes("pg_catalog.") && !stmt.includes("CREATE EXTENSION"))
      continue;
    ...
    try {
      ...
      if (success % 50 === 0 || i === statements.length - 1) {
        process.stdout.write(
          `\r   Executados: ${success} | Erros: ${errors} | Total: ${i + 1}/${statements.length}`,
        );
      }
    } catch (e) {
      errors++;
  ...
  for (const table of tablesToCheck) {
    try {
      const count =
        await sql`SELECT COUNT(*) as total FROM ${sql.identifier([table])}`;
