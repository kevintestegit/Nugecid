...
export const useAddDesarquivamentoComment = (id: number | null | undefined) => {
  ...
  return useMutation({
    mutationFn: (comment: string) =>
      apiService.addDesarquivamentoComment(numericId, comment),
    onSuccess: () => {
...
export const useUpdateDesarquivamento = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: [DESARQUIVAMENTOS_QUERY_KEY, variables.id],
      });
      queryClient.invalidateQueries({ queryKey: [DESARQUIVAMENTOS_QUERY_KEY] });
    },
  });
...
export const useRestoreDesarquivamento = () => {
  ...
  return useMutation({
    ...
    onError: (error: Error, restoredId) => {
      // Erro silencioso - apenas para tratamento
    },
  });
};
...
const extractTermoErrorMessage = async (error: unknown): Promise<string> => {
  ...
  if (axios.isAxiosError(error) && error.response?.data) {
    ...
    if (typeof data === "object" && data !== null && "message" in data) {
      return ((data as Record<string, unknown>).message as string) ?? fallback;
    }
  }
