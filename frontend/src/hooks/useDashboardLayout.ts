    ...
    const loadLayout = async () => {
      try {
        ...
        if (
          ...
          "cards" in (serverLayout as Record<string, unknown>)
        ) {
          const layout = serverLayout as DashboardLayout;
  ...
  const saveLayout = useCallback(
    async (newCards: DashboardCard[]) => {
      ...
      try {
        ...
        toast.success(
          "Layout salvo!",
          "Suas personalizações foram salvas com sucesso.",
        );
      } catch (error) {
        console.error("Erro ao salvar layout no servidor:", error);
        toast.warning(
          "Layout salvo localmente",
