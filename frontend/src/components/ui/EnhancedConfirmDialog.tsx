      ...
      <div className="fixed inset-0 z-[1000] flex items-center justify-center pointer-events-none p-4">
        ...
        >
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <CardTitle id="dialog-title" className="flex items-center gap-3">
                {showIcon && <Icon className={cn('h-6 w-6', styles.iconColor)} />}
                {title}
              </CardTitle>
              <Button
                variant="ghost"
          ...
          <CardContent className="space-y-4">
            <p id="dialog-description" className="text-muted-foreground leading-relaxed">{description}</p>

            {/* Warning List */}
            {warningList.length > 0 && (
              <div className={cn('rounded-lg border p-4', styles.bgColor, styles.borderColor)}>
            ...
            {confirmationType === 'text' && (
              <div className="space-y-2">
                <label htmlFor="confirm-input" className="block text-sm font-medium text-foreground">
                  ...
                  <span className={cn('font-bold', styles.iconColor)}>{confirmationKeyword}</span>{' '}
                  no campo abaixo:
                </label>
                <input
                  id="confirm-input"
            ...
            {confirmationType === 'countdown' && countdown > 0 && (
              <div className={cn('rounded-lg border p-4 text-center', styles.bgColor, styles.borderColor)}>
                <p className="text-sm text-muted-foreground mb-2">
                  Aguarde antes de confirmar
                </p>
                <p className={cn('text-3xl font-bold', styles.iconColor)}>{countdown}</p>
              </div>
            )}
