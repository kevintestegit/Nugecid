    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {details && (
            <p className={cn("text-sm mt-2", config.textColor)}>{details}</p>
          )}

          {suggestion && (
            <div
              className={cn(
                "mt-3 p-2 rounded bg-background/50 border",
                config.borderColor,
              )}
            >
...
export const getErrorMessage = (
  ...
  if (status === 400) {
    return {
      ...
      message: (data?.message as string) || "Os dados enviados são inválidos.",
