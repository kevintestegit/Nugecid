...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  const filteredTarefas = useMemo(() => {
    return tarefasList.filter((tarefa) => {
      ...
      const matchesPriority =
        !selectedPriority || tarefa.prioridade === selectedPriority;
  ...
  const getTaskColumnId = useCallback((tarefa: Tarefa): number => {
  ...
  const projectUsers = useMemo(() => {
    return (
      projeto.membros?.map((m) => m.usuario).filter((u): u is Usuario => !!u) ||
      []
    );
  }, [projeto.membros]);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (
      ...
      const sourceColumn = colunasById[sourceColunaId];
    ...
    if (
      ...
      if (activeColunaId === overColunaId) {
        ...
        const sourceColumn = colunasById[activeColunaId];
        const targetColumn = colunasById[overColunaId];
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-6 mb-2">
          <div className="max-w-2xl">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4 text-sm leading-relaxed">
              {projeto.descricao ||
                "Gerencie o fluxo de trabalho e atividades deste projeto."}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
              <div className="flex items-center gap-2" title="Data de criação">
                ...
                <span>
                  Criado em{" "}
                  {safeDate(projeto.data_criacao, "d 'de' MMMM, yyyy")}
                </span>
              </div>
        ...
        {sortedColunas.length === 0 ? (
          ...
          >
            <div
              className={cn(
                "flex h-full",
                density === "compact" ? "space-x-4" : "space-x-6",
              )}
            >
