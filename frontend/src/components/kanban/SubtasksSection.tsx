      ...
      <div className="space-y-2">
        {subtasks.map((task) => (
          <div
            ...
            className="flex items-center gap-3 p-3 bg-background border border-border rounded-lg hover:border-blue-300 hover:shadow-sm transition-all cursor-pointer group"
          >
            <div
              className={`w-2 h-2 rounded-full bg-${task.prioridade === "alta" ? "orange" : task.prioridade === "critica" ? "red" : "green"}-500 shrink-0`}
            />

            <span className="text-sm text-foreground/90 font-medium flex-1 truncate">
              {task.titulo}
            </span>
            ...
            <Badge
              variant="outline"
              className="text-[10px] px-1.5 py-0 h-5 bg-gray-50 border-border text-muted-foreground"
            >
