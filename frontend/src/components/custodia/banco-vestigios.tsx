...
import { Search, Filter, Trash2, Eye, RefreshCw, Printer, CheckSquare } from 'lucide-react'
...
const BancoVestigios: React.FC = () => {
  ...
  const [selectedVestigio, setSelectedVestigio] = useState<Vestigio | null>(null)
  const [dialogOpen, setDialogOpen] = useState(false)
  const [selectedForPrint, setSelectedForPrint] = useState<Set<string>>(new Set())
  const [printLayout, setPrintLayout] = useState<'one-per-page' | 'multiple'>('multiple')
  ...
  const fetchVestigios = useCallback(async () => {
    ...
    try {
      ...
      console.log('Vestígios extraídos:', vestígiosData)
      console.log('Is vestígiosData array?', Array.isArray(vestígiosData))
  ...
  const handlePrintSelected = () => {
    ...
    const selectedVestigios = vestigios.filter(v => selectedForPrint.has(v.id))
    
    const QR_CODE_DATA_URL = 'data:image/png;base64,'
    ...
    const etiquetasHtml = selectedVestigios.map((vestigio, index) => {
      const needsPageBreak = isOnePerPage && index < selectedVestigios.length - 1
  ...
  const categorias = useMemo(() => {
    ...
    return Array.from(new Set(vestigios.map((v) => v.categoria).filter(Boolean)))
  }, [vestigios])
          ...
          {selectedForPrint.size > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Select value={printLayout} onValueChange={(value: 'one-per-page' | 'multiple') => setPrintLayout(value)}>
                  <SelectTrigger className="w-48">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="multiple">Múltiplas por página</SelectItem>
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      checked={selectedForPrint.size === filteredVestigios.length && filteredVestigios.length > 0}
                    ...
                    <TableRow key={vestigio.id}>
                      <TableCell>
                        <Checkbox
                          ...
                          onCheckedChange={() => handleToggleSelection(vestigio.id)}
                      ...
                      <TableCell>
                        {new Date(vestigio.createdAt).toLocaleDateString('pt-BR')}
                      </TableCell>
          ...
          {selectedVestigio && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label className="text-muted-foreground">Número Vestígio</Label>
                ...
                <div>
                  <Label className="text-muted-foreground">Mês Referência</Label>
                ...
                <div className="col-span-2">
                  <Label className="text-muted-foreground">Etiqueta Completa</Label>
                ...
                <div className="col-span-2 border-t pt-4">
                  ...
                  <p>
                    ...
                    {new Date(selectedVestigio.createdAt).toLocaleString('pt-BR')}
                  </p>
                </div>
              </div>
            </div>
